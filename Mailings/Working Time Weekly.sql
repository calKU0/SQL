...
declare
	@dataOd date = CONVERT(DATE, DATEADD(WEEK, DATEDIFF(WEEK, 0, getdate()) - 1, 0))
	,@dataDo date = CONVERT(DATE, DATEADD(WEEK, DATEDIFF(WEEK, 0, getdate()), 0) - 1)



set @Temat = 'Produkcja mailing tygodniowy czasy od ' + convert(varchar(50), @dataOd) + ' do ' + convert(varchar(50), @dataDo)
...
select
	...
	,isnull(
		LTRIM(
		CONVERT(varchar, DATEDIFF(hour, '1900-01-01', CONVERT(datetime, DATEADD(ss, SUM(DATEDIFF(ss, '00:00:00.000', convert(time(0), DATEADD(ss, DATEDIFF(ss, 0, convert(time, DATEADD(ss,PCZ_CzasRealizacji / case when ISNULL(sa.Atr_Wartosc,'') = 'TAK' then 2 else 1 end,0))), 0)))), '00:00:00.000'))))
	) + ' h ' + 
	LTRIM(
		CONVERT(varchar, 
			DATEDIFF(minute, 0, 
				DATEADD(ss, SUM(DATEDIFF(ss, '00:00:00.000', convert(time(0), DATEADD(ss, DATEDIFF(ss, 0, convert(time, DATEADD(ss,PCZ_CzasRealizacji / case when ISNULL(sa.Atr_Wartosc,'') = 'TAK' then 2 else 1 end,0))), 0)))), '00:00:00.000'))
			% 60
		)
	...
	left join cdn.prodobiekty pob1 with(nolock) on prc1.Prc_GIDNumer=pob1.POB_ObiNumer and pob1.POB_ObiTyp=944-- pracownik
	left join cdn.ProdCzynnosciObiekty pco1 with(nolock)  on pob1.POB_Id=pco1.PCO_Obiekt
	left join cdn.prodczynnosci pcz1 with(nolock) on pcz1.PCZ_Id=pco1.PCO_Czynnosc and DATEDIFF(DD,'18001228',convert(date, Dateadd(Second, PCZ_TerminZakonczenia , '1990-01-01'))) between DATEDIFF(DD,'18001228',@dataOd) and DATEDIFF(DD,'18001228', @dataDo)
	left join cdn.prodprocesy ppc1 with(nolock) on ppc1.PPC_Id=pcz1.PCZ_Proces 
	left join cdn.prodzlecenia pzl1 with(nolock) on pzl1.PZL_Id=ppc1.PPC_Zlecenie
	left join cdn.ProdTechnologia pte1 with(nolock) on  pte1.PTE_Id=ppc1.PPC_Technologia
	left join cdn.ProdTechnologiaCzynnosci ptc1 with(nolock)   on pte1.PTE_Id=ptc1.PTC_Technologia and pcz1.PCZ_TechnologiaCzynnosc = ptc1.PTC_Id
